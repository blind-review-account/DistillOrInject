...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
CONFIG_BASE="${SCRIPT_DIR}/configs/java250/Inject"
#DATA_DIR="${SCRIPT_DIR}/data"
DATA_DIR="/network/scratch/l/luansito/data"


# ——— Download & unpack data if missing ———
if [ ! -d "${DATA_DIR}" ]; then
  echo "Data folder not found. Downloading from Google Drive…"
  cd "${SCRIPT_DIR}"
  ...
  # Download by file ID and unzip
  gdown --id 1UUTHv6Z0on0LgD4kSa24NIaOPQxsCoAH -O data.zip
